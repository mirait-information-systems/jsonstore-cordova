...
constant = (function() {
  ...
  /**
    Error Codes - Make sure you update geterrormessage.tests.js
...
@private
**/
JSONStoreUtil.check = (function(jQuery, underscore) {
  ...
  //TODO: need to find an elegant solution to check if device
  var __isDevice = function(){
  ...
  //TODO: this might not work since userAgents might not give correct result
  var __isWindows = function() {
    ...
    for (sortObjKey in sortObj) {
      if (sortObj.hasOwnProperty(sortObjKey)) {
        if (propertiesValidated > 0) {
          console.trace('Sort object ' + JSON.stringify(sortObj) + ' has more than one property. Each object must have only one property.')
          return false;
        }
        ...
        if (_.isUndefined(searchFields[sortObjKey.toLowerCase()]) && _.isUndefined(additionalSearchFields[sortObjKey.toLowerCase()])) {
          console.trace('Sort key ' + sortObjKey + ' is not part of search fields: ' + JSON.stringify(searchFields) + ' or additional search fields: ' + JSON.stringify(additionalSearchFields))
          return false;
        }
        ...
        if (__isString(sortStr) && sortStr.length > 0 && (/^(a|de)sc$/i.test(sortStr))) {
          console.trace('Sort key ' + sortObjKey + 'is not one of the valid strings.');
          propertiesValidated++;
        } else {
          return false;
        }
...
JSONStoreUtil.callback = (function(jQuery) {
  ...
  ErrorObject = function(obj) {
    ...
    this.msg = JSONStore.getErrorMessage(this.err);
    this.col = obj.col || '';
  ...
  var _generate = function(options, events, src, collectionName, username, deferred) {
    ...
    failure = function(data, more) {
      ...
      if (check.isInt(data)) {
        ...
        errorObject.msg = JSONStore.getErrorMessage(data);
      } else {
        //We got a document back instead of an error code
        errorObject.doc = data;
        errorObject.err = -11;
        errorObject.msg = JSONStore.getErrorMessage(-11);
      }
      ...
      //Need to create a better logger
...
@private
**/
JSONStoreUtil.findIterator = (function(underscore) {
...
if (JSONStoreUtil.check.isDevice()) {

  /**
  Provides access to our internal storage via cordova.
  Internally used in JSONStore.
  @private
  **/
  JSONStoreUtil.db = (function(jQuery) {
    ...
    var store = new Store(),
      ...
      __queryProcessorAdvImpl = function(doc) {
        ...
        for (var k in query) {
          if (hasOwn.call(query, k)) {
            ...
            iteratorImpl = JSONStoreUtil.findIterator.get(kLower);
      ...
      _change = function(collection, data, options) {
        ...
        if (data.length === 0) {
          ...
          JSONStore.get(col.name).find(retData, {
            exact: true
          })
            .then(function(res) {
              if (res.length > 0 && !check.isUndefined(ctx.replaceCriteria)) {
                ...
                return JSONStore.get(col.name).replace(replaceData, {
            ...
            .then(function(res) {
              ...
              if (ctx.addNew) {
                ...
                return JSONStore.get(col.name).add(newData, {
                  markDirty: ctx.markDirty
                });
              } else {
                var deferred = $.Deferred();
...
_JSONStoreImpl = (function(jQuery, underscore) {
  ...
  /**
    ...
    @private
    */
  var db = JSONStoreUtil.db || JSONStoreUtil.browser,
    cb = JSONStoreUtil.callback,
    ...
    COLLECTIONS = {},
    EVENT_LABELS = {
      success: 'JSONSTORE/SUCCESS',
      failure: 'JSONSTORE/FAILURE'
    },
    ...
    */
    ...
    __handleErrorReturn = function(errObj) {
      ...
      e.msg = JSONStore.getErrorMessage(errObj.errCode);
    ...
    __logDeprecatedMessage = function(old, alt) {
      console.debug('[Deprecated] ' + old + ', use ' + alt + ' instead.')
    },
    ...
    _usePassword = function(pwd) {
      ...
      __logDeprecatedMessage('JSONStore.usePassword', 'JSONStore.init');
    ...
    _clearPassword = function() {
      ...
      __logDeprecatedMessage('JSONStore.clearPassword', 'JSONStore.init');
      ...
      var arrp = [],
        ...
        errorFoundReject = function(faultyPromise, col) {
          ...
          def.reject(new ErrorObject({
            err: errCode,
            msg: JSONStore.getErrorMessage(errCode),
            src: 'initCollection',
            usr: options.username || constant.DEFAULT_USERNAME,
            col: col
          }));
        };
  ...
  JSONStoreInstance.prototype = {
    ...
    /**
      Prints the contents of the collection using console.debug asynchronously.
      */
    toString: function(limit, offset) {
      ...
      if (check.isUndefined(limit) && check.isUndefined(offset)) {
        ...
        col.findAll({
          ...
          console.debug("PKG - " + constant.PKG_NAME + " " + output);
          deferred.resolve(output);
        }).fail(function(err) {
          console.error("PKG - " + constant.PKG_NAME + " " + err);
        ...
        setTimeout(function() {
          ...
          output.collection.searchFields[constant.ID_KEY] = 'number';
          console.debug("PKG - " + constant.PKG_NAME + " " + output)
          console.log()
          deferred.resolve(output);
        }, 0);
        ...
        .fail(function(err) {
          console.debug("PKG - " + constant.PKG_NAME + " " + err)
          deferred.resolve(err);
        });
      }
